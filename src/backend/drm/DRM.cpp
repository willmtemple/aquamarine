...
IOutput::SParsedEDID Aquamarine::SDRMConnector::parseEDID(std::vector<uint8_t> data) {
    ...
    // First block of each kind wins, but keep scanning every extension: an EDID may carry more
    // than one CTA extension, and breaking out at the first one loses HDR or colorimetry data
    // that lives in a later block.
